...
import { KindBadge, DateCell } from '../components/ui/Badges'
...
export default function RelationsPage() {
  const api = useApi()
  const fetcher = useCallback(() => api.relations.list(), [api])
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">All relation records from <code className="font-mono text-xs bg-slate-800 px-1 rounded">/api/v1/relations</code></p>
        </div>
        <button
          onClick={refresh}

...
const COLUMNS = [
  ...
  { key: '_validUntilDateTime',label: 'Valid Until', render: (v) => <DateCell value={v} /> },
]

export default function EntitiesPage() {
  const api = useApi()
  const fetcher = useCallback(() => api.entities.list(), [api])
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">All entity records from <code className="font-mono text-xs bg-slate-800 px-1 rounded">/api/v1/entities</code></p>
        </div>
        <button
          onClick={refresh}

...
const COLUMNS = [
  ...
  { key: '_name',             label: 'Name',       render: (v) => <span className="font-medium text-slate-100">{v ?? '—'}</span> },
  { key: '_kind',             label: 'Kind',       render: (v) => <KindBadge value={v} /> },
  { key: '_visibility',      label: 'Visibility', render: (v) => <VisibilityBadge value={v} /> },
  { key: '_slug',             label: 'Slug',       render: (v) => <span className="font-mono text-xs text-slate-400">{v ?? '—'}</span> },
...
export default function ListsPage() {
  const api = useApi()
  const fetcher = useCallback(() => api.lists.list(), [api])
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-100">Lists</h2>
          <p className="text-sm text-slate-400 mt-0.5">All list records from <code className="font-mono text-xs bg-slate-800 px-1 rounded">/api/v1/lists</code></p>
        </div>
        <button
          onClick={refresh}

...
const COLUMNS = [
  ...
  { key: '_listId',          label: 'List ID',  render: (v) => <span className="font-mono text-xs text-slate-400">{v ?? '—'}</span> },
...
export default function ListReactionsPage() {
  const api = useApi()
  const fetcher = useCallback(() => api.listReactions.list(), [api])
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-100">List Reactions</h2>
          <p className="text-sm text-slate-400 mt-0.5">All list reactions from <code className="font-mono text-xs bg-slate-800 px-1 rounded">/api/v1/list-reactions</code></p>
        </div>
        <button
          onClick={refresh}
